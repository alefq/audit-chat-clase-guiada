 ...
 */
package com.alefq.java3.auditoria_chat.persistence;
...
import com.alefq.java3.auditoria_chat.domain.Bookmark;
...
public class BookmarkDAO extends JPACrud<Bookmark, Long> {
	...
	public List<Bookmark> findPage(int pageSize, int first, String sortField, boolean sortOrderAsc) {
		...
		Query q = em.createQuery("select c from ChatAudit c");
		List<ChatAudit> chatsAudits = q.getResultList();
		for (int i = 0; i < chatsAudits.size(); i++) {
			logger.info(chatsAudits.get(i).toString());
		}
